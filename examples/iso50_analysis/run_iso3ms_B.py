"""Solve the 3 ms calibration cross-check sample: arm B (no refit).
...
if len(sys.argv) > 3:
    RESP = sys.argv[3]
OD = sys.argv[4] if len(sys.argv) > 4 else f'{OUT}/iso50_3ms'
...
for i, t in enumerate(TAGS):
    ...
    seq = [
        {'LoadEvent': {'input': f'{NFS}/{t}_tred_nb1_3ms.npz', 'tpc': 0,
                       'max_events': 50}},
        ...
        {'WriteCharges': {'out_dir': f'{OD}/B/{t}', 'prefix': t}},
    ]
...
for i, (t, c) in enumerate(jobs, 1):
    os.makedirs(f'{OD}/B/{t}', exist_ok=True)
    marker = f'{OD}/B/{t}/{t}_event_0_49.npz'
    if os.path.exists(marker):
        print(f'[{i}] skip C/{t}', flush=True); continue
    if not os.path.exists(f'{NFS}/{t}_tred_nb1_3ms.npz'):
        print(f'[{i}] INPUT MISSING {t}', flush=True)
        continue
    cp = f'{OD}/B/job_{t}.yaml'
    with open(cp, 'w') as fh:
        yaml.safe_dump(c, fh, sort_keys=False)
    print(f'[{i}/{len(jobs)}] C/{t}', flush=True)
    with open(f'{OD}/B/{t}.log', 'w') as lg:
