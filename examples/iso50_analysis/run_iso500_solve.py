"""Solve the 3 ms calibration cross-check sample: arm C only.
...
TAGS = [l.strip() for l in open('/home/yousen/Documents/NDLAr2x2/MuonLArSim/iso500_list.txt') if l.strip()]
W, NW = int(sys.argv[1]), int(sys.argv[2])
if len(sys.argv) > 3:
    RESP = sys.argv[3]
OD = sys.argv[4] if len(sys.argv) > 4 else f'{OUT}/iso500cal'
...
for i, (t, c) in enumerate(jobs, 1):
    ...
    marker = f'{OD}/C/{t}/{t}_event_0_499.npz'
