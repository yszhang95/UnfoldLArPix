...
# Re-executes 8 parameter configurations (5 from ANALYSIS_LOG.md + 3 new)
...
echo "Configuration: 8 parameter combinations × 4 datasets"
echo ""
...
if [ -d "$DEST_DIR" ]; then
  ...
  echo "  JSON files: $json_count (expected: 64)"
  echo "  PNG plots:  $png_count (expected: 140)"
  echo "  Total files: $total_count (expected: 204+)"
  echo "  Size: $size"
  echo ""

  if [ "$json_count" -ge 60 ] && [ "$png_count" -ge 130 ]; then
    echo "✅ Analysis complete! All files generated successfully (TPC 0 only)"
  else
    echo "⚠️  Analysis in progress or incomplete"
  fi
else
...
echo "  Config 1: σ_temporal=0.005, σ_pixel=0.1   → 8 JSONs + 16 plots"
echo "  Config 2: σ_temporal=0.005, σ_pixel=0.2   → 8 JSONs + 20 plots"
echo "  Config 3: σ_temporal=0.004, σ_pixel=0.2   → 8 JSONs + 16 plots"
echo "  Config 4: σ_temporal=0.003, σ_pixel=0.2   → 8 JSONs + 16 plots"
echo "  Config 5: σ_temporal=0.003, σ_pixel=0.15  → 8 JSONs + 16 plots"
echo "  ───────────────────────────────────────────────────────────"
echo "  TOTAL: 64 JSONs + 140 plots = 204 files (estimated)"
echo ""
